...
sub new {
    my ($pkg, $subrule_calc) = @_;
    return unless ref $subrule_calc eq 'App::GUI::Cellgraph::Compute::Subrule';

    bless { subrules => $subrule_calc, subrule_result => [],
            output_list => [], output_pattern_index => {}, rule_number => 0,
...
sub renew {
    ...
    $self->{'max_rule_nr'} = ($self->{'subrules'}->state_count ** $self->{'subrules'}->independent_count) - 1;
    ...
    for my $i (0 .. $self->{'max_rule_nr'} - 1){
        ...
        $self->{'output_pattern_index'}{ $self->{'input_pattern'}[$i] } = $i;
    }
