...
sub new {
    ...
    $self->{'red'}   =  App::GUI::Cellgraph::SliderCombo->new( $self, 100, ' R  ', "red part of $type color",    0, 255,  0);
    $self->{'green'} =  App::GUI::Cellgraph::SliderCombo->new( $self, 100, ' G  ', "green part of $type color",  0, 255,  0);
    $self->{'blue'}  =  App::GUI::Cellgraph::SliderCombo->new( $self, 100, ' B  ', "blue part of $type color",   0, 255,  0);
    $self->{'hue'}   =  App::GUI::Cellgraph::SliderCombo->new( $self, 100, ' H  ', "hue of $type color",         0, 359,  0);
    $self->{'sat'}   =  App::GUI::Cellgraph::SliderCombo->new( $self, 100, ' S  ', "saturation of $type color",  0, 100,  0);
    $self->{'light'} =  App::GUI::Cellgraph::SliderCombo->new( $self, 100, ' L  ', "lightness of $type color",   0, 100,  0);
    $self->{'display'}= App::GUI::Cellgraph::ColorDisplay->new( $self, 25, 10, $init);
    $self->{'display'}->SetToolTip("$type color monitor");
    
    my $rgb2hsl = sub {
        my @rgb = ($self->{'red'}->GetValue, $self->{'green'}->GetValue, $self->{'blue'}->GetValue);
        my @hsl = App::GUI::Cellgraph::Color::Value::hsl_from_rgb( @rgb );
    ...
    my $hsl2rgb = sub {
        my @rgb = App::GUI::Cellgraph::Color::Value::rgb_from_hsl( 
