...
sub new {
    ...
    $self->{'frequency'}  = App::GUI::Cellgraph::SliderCombo->new
                        ( $self, 100, 'f', 'frequency of '.$help, 1, $max, 1 );
    $self->{'freq_dez'} = App::GUI::Cellgraph::SliderCombo->new
    ...
    $self->{'offset'} = App::GUI::Cellgraph::SliderCombo->new
                            ($self, 110, 'Offset', 'additional offset pendulum starts with (0 - quater rotation)', 0, 100, 0);
                            
    $self->{'radius'} = App::GUI::Cellgraph::SliderCombo->new( $self, 100, 'r', 'radius or amplitude of pendulum swing', 0, 150, 100);
    $self->{'damp'} = App::GUI::Cellgraph::SliderCombo->new( $self, 100, 'Damp', 'damping factor (diminishes amplitude over time)', 0, 1000, 0);
