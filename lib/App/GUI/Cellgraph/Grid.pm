...
sub get {
    ...
    my @start_states = @{ $config->{'start'}{'list'} };
...
say int @start_states;
    for my $row_i (1 .. $size_y - 1) { # compute next rows
        say int @{$state_grid->[$row_i - 1]};
        ($state_grid->[$row_i], $action_grid->[$row_i]) = $iterator->( $state_grid->[$row_i - 1], $action_grid->[$row_i - 1]);
    }    
        #~ my $row_before = $state_grid->[$row_i - 1];
        #~ my $action_before = $action_grid->[$row_i-1];
...
sub compile_iterator {
    ...
    # iter row
    # row end vals
    # my $value = $config->{'global'}{'input_size'} % 2 ? $row_states->[0] : 0;
    # $config->{'rules'}{'avg'}
say $code;
