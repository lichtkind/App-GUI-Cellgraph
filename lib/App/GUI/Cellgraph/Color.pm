...
package App::GUI::Cellgraph::Color;

use Carp;
use App::GUI::Cellgraph::Color::Constant ':all';
use App::GUI::Cellgraph::Color::Value ':all';
...
sub _rgb_from_name_or_hex {
    ...
    if (substr($arg, 0, 1) eq '#'){                  # resolve #RRGGBB -> ($r, $g, $b)
        ...
        carp "'$arg' is an unknown color name, please check App::GUI::Cellgraph::Color::Constant::all_names()." unless @rgb == 3;
        @rgb;
    }
}
...
sub add {
    ...
    my @hsl = App::GUI::Cellgraph::Color::Value::_hsl_from_rgb( @rgb ); # withound rounding
